...
ggplot(preprocess, aes(y=value, x=Samples, fill=metric)) +
  ...
  scale_fill_manual("",
    labels=c("Host/Contaminant", "Low Quality", "Retained"),
    values=c("grey60", "grey80", "#029ae5ff")) +
  theme_classic() +
  labs(
    title="Contaminant/low-quality read percentage",
    x="Sample", 
...
phyla.to.keep <- filter_taxon(phyla.hmap.dat)
phyla.hmap <- ggplot(phyla.hmap.dat %>% filter(MinRank %in% phyla.to.keep), aes(SampleID, MinRank, fill=log10(count))) +
...
phyla.dims <- eval(heatmap_dims(phyla.hmap))
p.height <- max(1.5+0.1*phyla.dims$nrows, 5)
p.width <- max(2.5+0.2*phyla.dims$ncols, 5)
test <- 5

family.hmap.dat <- MakeHeatmapData(agg, yaxis="Family", min.samples = max(1, n_samples*0.2))
family.to.keep <- filter_taxon(family.hmap.dat)
family.hmap <- ggplot(family.hmap.dat %>% filter(MinRank %in% family.to.keep), aes(SampleID, MinRank, fill=log10(count))) +
  geom_tile(color="grey80", size=0.4) +
  viridis::scale_fill_viridis(na.value="white", option = "A", labels=function(x) 10^(as.numeric(x))) +
  theme_classic() +
  theme(
...
family.dims <- heatmap_dims(family.hmap)

genera.hmap.dat <- MakeHeatmapData(agg, yaxis="Genus", min.samples = max(1, n_samples*0.2))
genera.to.keep <- filter_taxon(genera.hmap.dat, 50)
genera.hmap <- ggplot(genera.hmap.dat %>% filter(MinRank %in% genera.to.keep), aes(SampleID, MinRank, fill=log10(count))) +
  geom_tile(color="grey80", size=0.4) +
  viridis::scale_fill_viridis(na.value="white", option = "A", labels=function(x) 10^(as.numeric(x))) +
  theme_classic() +
  theme(
...
genera.dims <- heatmap_dims(genera.hmap)
species.hmap.dat <- MakeHeatmapData(agg, yaxis="Species", min.samples=0)
species.to.keep <- filter_taxon(species.hmap.dat, 50)
species.hmap <- ggplot(species.hmap.dat %>% filter(MinRank %in% species.to.keep), aes(SampleID, MinRank, fill=log10(count))) +
  geom_tile(color="grey80", size=0.4) +
  viridis::scale_fill_viridis(na.value="white", option = "A", labels=function(x) 10^(as.numeric(x))) +
  theme_classic() +
  theme(
...
species.dims <- heatmap_dims(species.hmap)
